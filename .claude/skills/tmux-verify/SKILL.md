...
- **vhs dies with `No usable sandbox!` on a Linux host.** vhs drives headless Chromium to rasterise the terminal, and on a box whose kernel or container denies the user namespaces Chromium's sandbox needs, it aborts before recording a single frame. Nothing is written, so it reads as a tape defect rather than a host one. Export `VHS_NO_SANDBOX=true` for the run. Dropping the sandbox is acceptable here precisely because the page being rendered is our own tape and nothing else, but keep it scoped to the recording command rather than the shell's environment.
- **A failed vhs run leaks its `ttyd`.** vhs starts a `ttyd` to host the terminal and only reaps it on a clean exit; a tape that fails to compile, or a vhs killed part-way, leaves the process alive holding its port, and the next run then records against the stale one. Find it by start time (`ps -eo pid,lstart,command | grep ttyd`) and confirm the timestamp matches the run you just lost, then end that one **exact PID**. Never `pkill ttyd` or `killall ttyd`: on a shared box you will take out another agent's recording, and the same rule that governs tmux sessions in G1 governs this.
- **An isolated `HOME` does not isolate tmux.** The seed gives the journey its own config, but the tmux server is per user, not per `HOME`, so a recording made while any other tripwire or dev session is running renders the host's unrelated sessions into the list under "Other tmux". The frames are then genuinely wrong for the journey and a frame-truth read will rightly reject them. Either record when nothing else holds a session, or pin the tape onto a private server with its own `TMUX_TMPDIR`, the way the tmux suites do. Two recordings must never overlap.
