...
| T5 | `DeviceScope { base, admin }`; admin implies base desktop (constructor and decoder) | `crates/ainb-hangar-proto/src/devices.rs` |
...
| T9 | Close codes 1013, 4401, 4403, 4409, 4429, 4503 | `crates/ainb-hangar-proto/src/peer_close.rs` |
| T13 | `SurfaceKind::Mobile` | `crates/ainb-hangar-proto/src/connections.rs` |
...
|---|---|---|
| M1 | `device/redeem` | `DeviceRedeemParams`, `DeviceRedeemResult` |
...
`ALL_METHODS`. `terminal/frame` is a notification, so it sits in
...
| C3 | Per-scope method and event table, explicit verdict per method per scope, NO default | `devices.rs` (`SCOPE_TABLE`, `EVENT_TABLE`), committed `crates/ainb-hangar-proto/scopes.catalogue` |
...
| C6 | Mobile event families: Fleet, Attention, Transcript | `EVENT_TABLE` |
