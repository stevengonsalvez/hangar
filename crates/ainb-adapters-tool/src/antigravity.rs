//! Antigravity adapter — Google Antigravity CLI layout (`~/.gemini/antigravity-cli`).
//!
//! Antigravity accepts `skill` and `agent`; declines every other kind
//! with a recorded reason so the lockfile entry shows `status: skipped`.
...
impl ToolAdapter for AntigravityAdapter {
    ...
    fn accepts(&self, kind: UnitKind) -> AcceptDecision {
        match kind {
            ...
            UnitKind::Plugin => AcceptDecision::No {
                reason: "antigravity does not support plugins (claude-native)".into(),
            },
            UnitKind::Command => AcceptDecision::No {
                reason: "antigravity does not support custom commands".into(),
            },
            UnitKind::Hook => AcceptDecision::No {
                reason: "antigravity does not support hooks".into(),
            },
            UnitKind::McpServer => AcceptDecision::No {
                reason: "antigravity does not support MCP servers".into(),
            },
            UnitKind::Statusline => AcceptDecision::No {
                reason: "antigravity does not support statuslines".into(),
            },
        }
    }
    ...
    fn template_substitutions(&self) -> HashMap<&'static str, String> {
        ...
        m.insert("TOOL_DIR", ".gemini/antigravity-cli".to_string());
        m.insert("TOOL_NAME", "antigravity".to_string());
        m.insert("HOME_TOOL_DIR", "~/.gemini/antigravity-cli".to_string());
        m
    }
}
