//! Learnings plugin binary entry point.
//!
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Send tracing output to stderr so it doesn't pollute the JSON-RPC
    // framing on stdout. The runtime forwards stderr lines into its host
    // log.
    ...
    Server::new(LearningsPlugin::default()).run_stdio().await?;
    Ok(())
}
